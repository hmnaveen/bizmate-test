    ...
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
                        ...
                        <div class="form-input--wrap">
                            ...
                            <div class="row">
                                <div class="col-12">
                                    ...
                                    <!--<div class="btn-group sumb--dashboardDropdown transaction--filter invoice-expenses--dropdown" role="group">
                                        <button id="invoice_item_tax_rate" name="invoice_item_tax_rate" type="button" data-toggle="dropdown" aria-expanded="false" value="">
                                            Tax Rate Option
                                        </button>
                                        <div class="dropdown-menu" aria-labelledby="btnGroupDrop_type">

                                            @foreach($tax_rates as $tax_rate)
                                                <a class="dropdown-item" href="javascript:void(0)" hidden="hidden" id="invoice_tax_rate_{{$tax_rate['id']}}" >{{$tax_rate['tax_rates']}}</a>
                                                <a class="dropdown-item" href="javascript:void(0)" id="invoice_tax_option_{{$tax_rate['id']}}"  onclick="getInvoiceTaxOptions('invoice_item_tax_rate', {{ $tax_rate['id'] }}, 'invoice_tax_option_{{ $tax_rate['id'] }}')">{{ $tax_rate['tax_rates_name'] }}</a>
                                            @endforeach
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary save--btn" onclick="addBasicInvoiceItem('invoice_item_part_row_id')">Save</button>
            </div>
        </div>
    </div>
</div>
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-secondary close--btn" data-dismiss="modal">Close</button>
                <button type="button" class="btn btn-primary save--btn" onclick="addBasicInvoiceNewAccount('invoice_account_part_row_id')">Save</button>
            </div>
        </div>
    </div>
</div> -->
...
<div class="modal fade modal-reskin modal-invoice--send" id="send_invoice_modal" tabindex="-1" role="dialog" aria-labelledby="send_invoice_modal" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
        <div class="modal-content send-invoice-modal">
            ...
            <form action="/basic/invoice/send-email?invoice_id={{$invoice_id}}"  method="post" enctype="multipart/form-data" id="send_invoice_pop_up">
            {{ method_field('POST') }}
            {{ csrf_field() }}  
            ...
            <div class="container">
                    ...
                    <div class="row">
                        <div class="col-xl-4 col-lg-4">
                            <ul class="list-unstyled">
                                ...
                                <li>Invoice Date: <span id="invoice_preview_issue_date"></span></li>
                                <li>Due Date: <span id="invoice_preview_due_date"></span></li>
                            </ul>
                        </div>
                        ...
                        <div class="col-xl-4 col-lg-4 invoicetable--header_from">
                            <img style="width:200px" src="{{!empty($invoice_settings) && $invoice_settings['business_logo'] ? '/uploads/'.$userinfo[0].'/'.$invoice_settings['business_logo'] : ''}}">
                        </div>
                    </div>
                </div>
                ...
                <div class="table-responsive mt-3">
                    <table class="table table-striped" id="invoice_preview_parts">
                        <thead>
                            <tr>
                                <th scope="col" style="width:135px; min-width:200px;">Item</th>
                ...
                <div class="row mt-4 invoice--extrainfo">
                    <div class="col-xl-8 col-lg-8">
                        ...
                        <p class="invoice--paymentnotes">
                            <?php if(!empty($invoice_settings)){?>
                                    {!! nl2br(e($invoice_settings['business_terms_conditions'])) !!}
                            <?php }?>
                        </p>
                    </div>
                    <div class="col-xl-4 col-lg-4">
                        <table class="table table-clear invoice--paymentinfo">
                            <tbody>
                                ...
                                <tr>
                                    <td>
                                        <strong id="invoice_preview_total_tax_percentage">Total Tax %</strong>
                                    </td>
            ...
            <div class="container-fluid" id="my-div-to-mask">
                <section>
                    @if(!empty($invoice_details && $invoice_details['status']) && $invoice_details['status'] == 'Voided' || !empty($invoice_details && $invoice_details['status']) && $invoice_details['status'] == 'Paid')
                        ...
                        <div class="invoice--status-deets">This invoice entry is on Read Only mode. Entries flagged as <u>{{!empty($invoice_details) ? $invoice_details['status'] : '' }}</u> cannot be edited.</div>
                ...
                <section id="testIngHide">
                    <form action="/basic/invoice-create-save?invoice_id={{$invoice_id}}&type={{$type}}" method="post" enctype="multipart/form-data" class="form-horizontal" id="invoice_form">
                        @csrf
                        ...
                        <div class="row">
                            ...
                            <div class="col-xl-4">
                                ...
                                <div class="form-input--wrap">
                                    ...
                                    <div class="row">
                                        <div class="col-12">
                                            <select class="form-input--dropdown" id="invoice_default_tax" name="invoice_default_tax" value="" onchange="basicInvoicepartsQuantity('invoice_default_tax')">
                                    ...
                                    @if (!empty($invoice_details))
                                        ...
                                        <tr id="{{'invoice_parts_row_id_'.$row_index}}" class="invoice_parts_form_cls" >
                                            <td>
                                                ...
                                                <input placeholder="Search your item list" autocomplete="off" data-toggle="dropdown" id="{{'invoice_parts_name_code_'.$row_index}}" name="{{'invoice_parts_name_code_'.$row_index}}" type="text" onkeyup="searchBasicInvoiceparts(this, '{{$row_index}}')" value="{{!empty($invoice_part_code_name) ? $invoice_part_code_name : ''}}" required>
                                                ...
                                                <ul class="search_items_{{$row_index}} dropdown-menu invoice-expenses--dropdown" id="{{'invoice_item_list_'.$row_index}}" >
                                                    <li class="add-new--btn">
                                                        <a href="#" data-toggle="modal" data-target="#newItemModal" onclick="openBasicInvoicePopUpModel('{{$row_index}}')"><i class="fa-solid fa-circle-plus"></i>New Item</a>
                                                    </li>
                                                    @if (!empty($invoice_items))
                                                        ...
                                                        @foreach ($invoice_items as $item)
                                                            @php $counter ++; @endphp
                                                            <li>
                                                                <button type="button" class="invoice_item" data-myid="{{ $counter }}" onclick="getBasicInvoiceItemsById('{{ $item['id'] }}', '{{$row_index}}')">
                                            ...
                                            <td>
                                                <input id="{{'invoice_parts_quantity_'.$row_index}}" name="{{'invoice_parts_quantity_'.$row_index}}" type="number" onchange="basicInvoicepartsQuantity('{{$row_index}}')" value="{{!empty($parts['parts_quantity']) ? $parts['parts_quantity'] : 0 }}" required>
                                                @error('parts_quantity_'.$row_index)
                                                    <div class="alert alert-danger">{{ $message }}</div>
                                                @enderror
                                            </td>
                                            ...
                                            <td>
                                                <input type="float" id="{{'invoice_parts_unit_price_'.$row_index}}" name="{{'invoice_parts_unit_price_'.$row_index}}"  value="{{!empty($parts['parts_unit_price']) ? number_format($parts['parts_unit_price'], 2)  : 0.00}}" onchange="basicInvoicepartsQuantity('{{$row_index}}')" onfocusin='basicInvoiceRemoveComma("invoice_parts_unit_price_{{$row_index}}")' onfocusout='basicInvoiceAddComma("invoice_parts_unit_price_{{$row_index}}")' step=".01" required>
                                                ...
                                                <ul class="dropdown-menu invoice-expenses--dropdown" id="{{'invoice_chart_account_list_'.$row_index}}" >
                                                    ...
                                                    @if (!empty($chart_account))
                                                        ...
                                                        @foreach ($chart_account as $item)
                                                                ...
                                                                <li>
                                                                    <button type="button" class="invoice_item" data-myid="{{ $counter }}" onclick="addBasicInvoiceChartAccount('{{ $particulars['id'] }}', '{{$row_index}}')">
                                                    ...
                                                    <div class="form-input--wrap">
                                                        <div class="row">
                                                            <div class="col-12 for--tables">
                                                                <select class="form-input--dropdown" id="{{'invoice_parts_tax_rate_'.$row_index}}" name="{{'invoice_parts_tax_rate_'.$row_index}}" onchange="basicInvoicepartsQuantity('{{$row_index}}'); getBasicTaxRates('{{$row_index}}');" value="" required style="display : {{!empty($invoice_details['default_tax']) && $invoice_details['default_tax']=="no_tax" ? 'none' : 'block' }}">
                                                                    <option selected value="0#|#0">Tax Rate Option</option>    
                                                                    @foreach($tax_rates as $tax_rate)
                                            ...
                                            <td class="tableOptions">
                                                <button class="btn sumb--btn delepart" type="button" onclick="deleteBasicInvoiceParts(<?php echo $row_index ?>)" ><i class="fas fa-trash-alt"></i></button>
                                            </td>
                                        </tr>
                                        ...
                                        @else
                                            <tr id="invoice_parts_row_id_0" class="invoice_parts_form_cls" >
                                                <td>
                                                    <input placeholder="Search your item list" autocomplete="off" data-toggle="dropdown" type="text" id="invoice_parts_name_code_0" name="invoice_parts_name_code_0" onkeyup="searchBasicInvoiceparts(this, 0)"  value="" required>
                                                    ...
                                                    <ul class="search_items_0 dropdown-menu invoice-expenses--dropdown" id="invoice_item_list_0">
                                                        
                                                        <li id="add_new_invoice_item_0" class="add-new--btn">
                                                            <a href="#" data-toggle="modal" data-target="#newItemModal" onclick="openBasicInvoicePopUpModel(0)"><i class="fa-solid fa-circle-plus"></i>New Item</a>
                                                        </li>
                                                        @if (!empty($invoice_items))
                                                            @php $counter = 0; @endphp
                                                            @foreach ($invoice_items as $item)
                                                                @php $counter ++; @endphp
                                                                <li>
                                                                    <button type="button" class="invoice_item" data-myid="{{ $counter }}" onclick="getBasicInvoiceItemsById('{{ $item['id'] }}', 0)">
                                                                        <span id="data_name_{{ $counter }}">{{ $item['invoice_item_code'] }} : {{ $item['invoice_item_name'] }}</span>
                                                                        <input type="hidden" id="invoice_item_id_{{ $counter }}" name="invoice_item_id" value="{{ $item['id'] }}">
                                                                    </button>
                                                                </li>
                                                ...
                                                <td>
                                                    <input id="invoice_parts_quantity_0" name="invoice_parts_quantity_0" type="number" onchange="basicInvoicepartsQuantity(0)" value="" required>
                                                </td>
                                                <td>
                                                    <textarea id="invoice_parts_description_0" name="invoice_parts_description_0" class="autoresizing" value="" required></textarea>
                                                </td>
                                                <td>
                                                    <input type="float" id="invoice_parts_unit_price_0" name="invoice_parts_unit_price_0"  value="" onchange="basicInvoicepartsQuantity(0)" onfocusout="basicInvoiceAddComma('invoice_parts_unit_price_0');" onfocusin="basicInvoiceRemoveComma('invoice_parts_unit_price_0');" >
                                                ...
                                                <td hidden="hidden">
                                                    ...
                                                    <ul class="dropdown-menu invoice-expenses--dropdown" id="invoice_chart_account_list_0">
                                                        ...
                                                        @foreach ($chart_account as $item)
                                                            ...
                                                            ?>
                                                            <li>
                                                                <button type="button" class="invoice_item" data-myid="{{ $counter }}" onclick="addBasicInvoiceChartAccount('{{ $particulars['id'] }}', 0)">
                                                                    <span id="data_name_{{ $counter }}">{{ $particulars['chart_accounts_particulars_code'] }} - {{ $particulars['chart_accounts_particulars_name'] }} </span>
                                                ...
                                                <td id="invoice_parts_tax_rate_td_0">
                                                    @if(!empty($tax_rates))
                                                        ...
                                                        <div class="form-input--wrap">
                                                            <div class="row">
                                                                <div class="col-12 for--tables">
                                                                    <select class="form-input--dropdown" id="invoice_parts_tax_rate_0" name="invoice_parts_tax_rate_0" onchange="basicInvoicepartsQuantity(0); getBasicTaxRates(0);" value="" required >
                                                                        <option selected value="0#|#0">Tax Rate Option</option>    
                                                                        @foreach($tax_rates as $tax_rate)
                                                ...
                                                <td>
                                                    <input class="input--readonly" readonly id="invoice_parts_amount_0" name="invoice_parts_amount_0" type="float" value="">
                                                </td>
                                                <td class="tableOptions">
                                                    <button class="btn sumb--btn delepart" type="button" onclick="deleteBasicInvoiceParts(0)" ><i class="fas fa-trash-alt"></i></button>
                                                </td>
                                            </tr>
                                        @endif
                                        ...
                                        <tr class="add--new-line" style="display: {{ empty($invoice_details['status']) || $invoice_details['status'] === 'Unpaid' ? '' : 'none' }}">
                                            <td colspan="8">
                                                <button class="btn sumb--btn" type="button" id="addnewline" onclick="addBasicInvoiceParts()" ><i class="fa-solid fa-circle-plus"></i>Add New Line</button> 
                                            </td>
                                        </tr>
                                        ...
                                        <?php if(!empty($invoice_details) && $invoice_details['amount_paid']>0){ ?>
                                            <tr class="invoice-total--paid">
                                                ...
                                                <td colspan="3">
                                                    <input type="float" id="" name="" readonly value="{{!empty($invoice_details) ? number_format($invoice_details['amount_paid'] + $invoice_details['total_amount'], 2) : 0 }}">
                                                </td>
                                            </tr>
                        ...
                        <div class="form-navigation">
                            <div class="form-navigation--btns row">
                                ...
                                <div class="col-xl-8 col-lg-10 col-md-9 col-sm-12 col-xs-12 col-12">
                                    ...
                                    <?php }?> -->
                                    <?php if($type == 'edit' && ($invoice_details['status'] == 'Unpaid' || ( $invoice_details['status'] == 'Recalled' && !$invoice_details['invoice_sent']))){?>
                                        <div class="sumb--fileSharebtn btn-group" id="">
                                            <button class="btn sumb--btn btn-sm dropdown-toggle" id="send_invoice_drop_down" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                                Email Invoice
                                            </button>
                                            <div class="dropdown-menu">
                                                <a class="dropdown-item sentemail--btn" href="#" id="mark_as_sent">Mark as sent</a>                                                
                                                <a class="dropdown-item send--btn" href="#" id="send_invoice" onclick="sendInvoice()">Send Invoice</a>
                                            </div>
                                        </div>
                                    <?php }?>
                                        
                                    <button type="reset" class="btn sumb--btn reset--btn"><i class="fa fa-ban"></i>Clear Invioce</button>
                                    <button type="button" class="btn sumb--btn preview--btn" onclick="previewBasicInvoice()"><i class="fa-solid fa-eye" ></i>Preview</button>
...
<script>
    ...
    <?php if(!empty($invoice_id)){?>
        $(document).on('click', '#mark_as_sent', function(event) {
            var url = "{{URL::to('/basic/invoice/{id}/email-status')}}";
