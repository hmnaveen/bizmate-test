            ...
            <div class="container-fluid" id="my-div-to-mask">
                <section>
                    @if(!empty($invoice_details && $invoice_details['status']) && $invoice_details['status'] == 'Voided' || !empty($invoice_details && $invoice_details['status']) && $invoice_details['status'] == 'Paid')
                        <h3 class="sumb--title">
                            @if( $invoice_details['status'] == 'receive_money' && $invoice_details['payment_option'] == 'pre_payment')
                                Pre Payment
                        ...
                        @elseif( $invoice_details['status'] == 'receive_money' && $invoice_details['payment_option'] == 'pre_payment')
                            <h3 class="sumb--title">Edit Pre Payment</h3>
                                <!-- {{ str_pad($invoice_details['transaction_number'], 6, '0', STR_PAD_LEFT) }} -->
                        @else 
                            <h3 class="sumb--title">Edit Over Payment</h3>
                    @endif

                </section>
                
                <section id="testIngHide">
                    <form action="/account/receivable/edit?invoice_id={{$invoice_id}}" method="post" enctype="multipart/form-data" class="form-horizontal" id="invoice_form">
                        @csrf
                                ...
                                <div class="col-xl-4">
                                    <div class="form-input--wrap">
                                        ...
                                        <div class="form--inputbox readOnly row">
                                            <div class="col-12">
                                                <input type="text" readonly="" id="invoice_number" name="" value="{{!empty($invoice_details && $invoice_details['transaction_number'])  ? 'INV-'.str_pad($invoice_details['transaction_number'], 6, '0', STR_PAD_LEFT) : 'INV-'.str_pad($transaction_number, 6, '0', STR_PAD_LEFT) }}">
                        ...
                        <div class="table-responsive">
                            <table id="partstable">
                                <thead>
                                    <tr>
                                        @if($invoice_details['payment_option'] == 'pre_payment')
                                ...
                                <tbody>
                                    ...
                                    @if (!empty($invoice_details))
                                        ...
                                        <tr id="{{'invoice_parts_row_id_'.$row_index}}" class="invoice_parts_form_cls" >
                                            ...
                                            <td>
                                                <input {{$invoice_details['payment_option'] == 'over_payment' ? 'disabled' : '' }} id="{{'invoice_parts_unit_price_'.$row_index}}" name="{{'invoice_parts_unit_price_'.$row_index}}" type="float" value="{{!empty($parts['parts_unit_price']) ? number_format($parts['parts_unit_price'], 2)  : 0.00}}" onchange="invoicepartsQuantity('{{$row_index}}')" onfocusin='removeComma("invoice_parts_unit_price_{{$row_index}}")' onfocusout='addComma("invoice_parts_unit_price_{{$row_index}}")' step=".01" required >
                                                    ...
                                                    <div class="form-input--wrap">
                                                        <div class="row">
                                                            <div class="col-12 for--tables">
                                                                <select {{$invoice_details['payment_option'] == 'over_payment' ? 'disabled' : '' }} class="form-input--dropdown" id="{{'invoice_parts_tax_rate_'.$row_index}}" name="{{'invoice_parts_tax_rate_'.$row_index}}" onchange="invoicepartsQuantity('{{$row_index}}'); getTaxRates('{{$row_index}}');" value="" required >
                                                                    <option selected value="0#|#0">Tax Rate Option</option>    
                                                                    @foreach($tax_rates as $tax_rate)
                                        ...
                                        @if($invoice_details['payment_option'] == 'pre_payment')
                                            <tr class="add--new-line" style="display: {{ empty($invoice_details['status']) || $invoice_details['status'] === 'Unpaid' ? '' : 'none' }}">
                                                <td colspan="8">
                                                    <button class="btn sumb--btn" type="button" id="addnewline" onclick="addInvoiceParts('{{$invoice_details['transaction_type']}}','{{$invoice_details['payment_option']}}')" ><i class="fa-solid fa-circle-plus"></i>Add New Line</button> 
                                                </td>
                                            </tr>
                                        @endif
