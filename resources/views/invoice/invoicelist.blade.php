            ...
            <div class="container-fluid">
                                                    ...
                                                    <ul>
                                                        @if(!empty($total_invoice_counts))
                                                            @foreach($total_invoice_counts as $invoice_count)
                                                                <li class="{{ $invoice_count['status'] }}">
                                                                    <span>{{$invoice_count['status_count']}}</span> {{$invoice_count['status']}} Invoice <u>${{!empty($invoice_count) ? number_format($invoice_count['total'], 2) : ''}}</u>
                                                                </li>
                                                            @endforeach
                                                        @endif
                                                    </ul>

                                                    <a href="/invoice/create" class="add--btn">Add New Invoice</a>
                                                </div>
                ...
                <section>
                    <div class="row">
                        ...
                        @endif
                            @isset($err)
                            <div class="sumb-alert alert alert-{{ $error[$err][1] }}" role="alert">
                                {{ $error[$err][0] }}
                            </div>
                            @endisset
                                ...
                                <div class="table-responsive">
                                    <table class="invoice_list">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                <td colspan="9" style="padding: 30px 15px; text-align:center;">No Data At This time.</td>
                                            </tr>
                                            @else
                                                @foreach ($invoicedata['data'] as $invoice)
                                                        <tr>
                                                            <td  onclick="redirectUri('{{ json_encode($invoice) }}')">{{ $invoice['issue_date'] }}</td>
                                                            <td  onclick="redirectUri('{{ json_encode($invoice) }}')">{{ 'INV-'. str_pad($invoice['transaction_number'], 6, '0', STR_PAD_LEFT) }}</td>
                                                            <td  onclick="redirectUri('{{ json_encode($invoice) }}')">{{ $invoice['client_name'] }}</td>
                                                            <td  onclick="redirectUri('{{ json_encode($invoice) }}')">{{ $invoice['payment_date'] }}</td>
                                                            <td  onclick="redirectUri('{{ json_encode($invoice) }}')"><span class="payment--status-{{$invoice['status']}}">{{ $invoice['status'] == 'PartlyPaid' ? 'Partial Paid' : $invoice['status'] }}</span></td>
                                                            <td  onclick="redirectUri('{{ json_encode($invoice) }}')">${{ number_format((float)$invoice['amount_paid'], 2, '.', ',') }}</td>
                                                            <td  onclick="redirectUri('{{ json_encode($invoice) }}')">${{ number_format((float)$invoice['total_amount'], 2, '.', ',') }}</td>
                                                            <td  onclick="redirectUri('{{ json_encode($invoice) }}')"><span class="{{ $invoice['invoice_sent'] ? 'email2client_sent' : 'email2client_pending' }}"></span></td>
...
<script>
    ...
    function redirectUri(data)
    {
       var data = JSON.parse(data);
        ...
        {
            location.href = "/invoice/"+data['id']+"/edit";
        }
        else if(data['transaction_type'] == 'arprepayment' || data['transaction_type'] == 'aroverpayment')
        {
            location.href = "/bank/cash-receipt?id="+data['id']+"&transaction_type="+data['transaction_type']
        }
    }
