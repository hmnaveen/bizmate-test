        ...
        <div class="section__content section__content--p30">
            <div class="container-fluid">
                ...
                @else
                    ...
                    <form action="/bank/account/transactions"  method="GET" enctype="multipart/form-data" id="transaction_search_form">
                                                ...
                                                <select class="form-input--dropdown" name="bank_account_id" id="bank_account_id">
                                                    @if(!empty($bank_accounts))
                                                        @foreach ($bank_accounts as $account)
                                                            <option {{ ( $account['account_id'] == $bank_account_id ) ? 'selected' : '' }} value="{{ $account['account_id'] }}">{{ $account['account_name'] }}</option>
                                                        @endforeach
                                                    @else
                                        ...
                                        <div class="col-12">
                                            <div class="transactionBox_wrap">
                                                <div class="transactionBox">
                                                    <div>
                                                        @php
                                                            $currentAccount = $bank_accounts->where('account_id','=', $bank_account_id)->first();
                                                        @endphp
                                ...
                                <div class="sumb--backAccountsBox sumb--dashstatbox sumb--putShadowbox">
                                    <div class="bankTransaction--status row" style="height: 100%">
                                        ...
                                        @php
                                            ...
                                            $DebitCount = DB::table('bank_transactions')->where('account_id','=',$bank_account_id)->where('direction','=','debit')->count();
                                            $CreditCount = DB::table('bank_transactions')->where('account_id','=',$bank_account_id)->where('direction','=','credit')->count();
                                            $ReconciledCount = DB::table('bank_transactions')->where('account_id','=', $bank_account_id)->where('is_reconciled','=', 1)->count();
                                            $UnreconciledCount = DB::table('bank_transactions')->where('account_id','=', $bank_account_id)->where('is_reconciled','=', 0)->count();
...
<script>
    ...
    function clearSearchItems(){
        ...
        var url = "{{URL::to('/bank/account/transactions?bank_account_id='.$bank_account_id)}}";
        location.href = url;
