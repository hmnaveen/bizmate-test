                        ...
                        @else
                            ...
                            @foreach($bank_transactions['data'] as $transaction)
                                ...
                                @foreach($appTransactions as $appTransaction)
                                    @if($appTransaction['total_amount'] == ($transaction['amount']) && (($appTransaction['transaction_type'] == 'invoice' && $transaction['direction'] == 'credit') || ($appTransaction['transaction_type'] == 'expense' && $transaction['direction'] == 'debit'))) 
                                        @php($match = 1)
                                        @break;
                                    @else
                                        @php($match = 0)
                                            ...
                                            <div class="RT--sumbTransaction">
                                                ...
                                                <div class="transactions">
                                                    ...
                                                    <ul class="nav nav-tabs" id="tabs_{{ $k }}">
                                                        ...
                                                        <li class="nav-item">
                                                            <a id="discuss_tab_{{ $k }}" class="nav-link tablinks_{{ $k }}" onclick="openTab(event, 'discuss_{{ $k }}','{{ $k }}','{{ $i }}')"><i class="fa-regular fa-file-lines"></i>Discuss</a>
                                                        </li>
                                                            ...
                                                            <div class="row">
                                                                <div class="col-12">
                                                                    <div class="form-input--wrap">
                                                                        ...
                                                                        <div class="form--recentsearch clientname_{{$k}} row">
                                                                            <div class="col-12">
                                                    ...
                                                    <div id="discuss_{{ $k }}" class="discussTransaction tabcontent_{{ $k }}">
                                                        <div class="col-12">
                                                            <div class="form-input--wrap textarea">
                                                                <label class="form-input--question">Reason</label>
                                                                <textarea class="col-12" id="transaction_discussion_{{ $k }}" name="transaction_discussion_{{ $k }}" onkeyup="discuss('transaction_discussion_{{ $k }}','{{ $k }}','{{ $transaction['id'] }}')"></textarea>
                                                            </div>
                                                        </div>
...
<script>
   ...
   $(document).ready(function () {
        ...
        const B = "The quick brown fox Naveen Jump from hill"
    ...
    function getClient(index, obj)
    {
        // $("#"+obj.id).on('keyup', function() {
            ...
            $('.form--recentsearch.clientname_'+index).show();
            var value = $("#"+obj.id).val().toLowerCase();
            
            var clientList = $(".clientname_"+index+ ".form--recentsearch__result li button");
            var matchedItems = $(".clientname_"+index+ " .form--recentsearch__result li button").filter(function() {
                return $(this).text().toLowerCase().indexOf(value) > -1;              
            });
            
            if(value == ''){
                $('.form--recentsearch.clientname_'+index).hide();
                $('.clientname_'+index+ 'li.add--newactclnt input').prop('checked',false);
                $("#"+obj.id).removeClass('saveNewRecord');

            } else if($("#"+obj.id).hasClass('saveNewRecord')) {

                if(matchedItems.length !=0) {
                    $('.clientname_'+index+ 'li.add--newactclnt').hide();
                    $('.clientname_'+index+ 'li.add--newactclnt input').prop('checked',false);
                    $("#"+obj.id).removeClass('saveNewRecord');
                ...
                if (matchedItems.length == 0) {
                    $('.clientname_'+index+ 'li.add--newactclnt').show();
                } else {
                    $('.clientname_'+index+ 'li.add--newactclnt input').prop('checked',false);
                    $('.clientname_'+index+ 'li.add--newactclnt').hide();
                }
    ...
    function discuss(id,index,transactionId)
    {
        var newText = $("#"+id).val();
        if(newText)
        {
            ...
            $("#reconcil_transaction_ok_btn_"+index).append(
                '<button id="transaction_discussion_btn_'+index+'" onclick=saveDiscussion('+encodeURI(index)+','+encodeURI(index)+') class="btn sumb--btn reconcile--btn" >Save</button>'
            );
        }
    }
    ...
    function saveTransaction(index, obj)
    {
        ...
        $.ajax({
            ...
            success: function(response){
                ...
                let total_matched = (parseFloat($("#bank_transaction_amount_"+index).val().replace(/\,/g,'')) - parseFloat($("#transaction_subtotal_"+index).val().replace(/\,/g,''))).toFixed(2);
                $("#total_matched_"+index).val(total_matched);

                $("#transaction_minor_adjustment_"+index).val('0');

                if(parseFloat($("#spent_receive_money_"+index).val().replace(/\,/g,'')).toFixed(2) != parseFloat($("#bank_transaction_amount_"+index).val().replace(/\,/g,'')).toFixed(2))
                {
    ...
    function addOrRemoveTransactions(rowIndex, id, transaction_type)
    {
        ...
        if(match_transaction_ele.checked)
        {
            ...
            let total_matched = (Number($("#bank_transaction_amount_"+rowIndex).val().replace(/\,/g,'')) - parseFloat($("#spent_receive_money_"+rowIndex).val().replace(/\,/g,''))).toFixed(2);
            ...
            if(parseFloat($("#spent_receive_money_"+rowIndex).val().replace(/\,/g,'')).toFixed(2) != parseFloat($("#bank_transaction_amount_"+rowIndex).val().replace(/\,/g,'')).toFixed(2))
            {
                $("#transactionReconcile_"+rowIndex).prop('disabled', true);
                $("#reconcile_matching_transactions_"+rowIndex).prop('disabled', true);
            }else{
                $("#transactionReconcile_"+rowIndex).prop('disabled', false);
                $("#reconcile_matching_transactions_"+rowIndex).prop('disabled', false);
            }
            ...
            let total_matched = (parseFloat($("#bank_transaction_amount_"+rowIndex).val().replace(/\,/g,'')) - parseFloat($("#spent_receive_money_"+rowIndex).val().replace(/\,/g,''))).toFixed(2);
            $("#total_matched_"+rowIndex).val(parseFloat(total_matched).toFixed(2));
    ...
    {
        const balance = Number($("#balance").val().replace(/\,/g,'')).toFixed(2);
        ...
        let part_payment = parseFloat($("#part_payment").val()).toFixed(2);
        
        if(parseFloat(part_payment) >= parseFloat(balance) || part_payment <= 0 || remaining_balance <= 0)
        {
           $(".close").click();
        }else
        {
            ...
            if(transaction.transaction_type == "invoice")
            {
                ...
                receive_money = parseFloat(remaining_balance).toFixed(2);;
            ...
            }else if(transaction.transaction_type == "expense")
            {
                ...
                spend_money = parseFloat(remaining_balance).toFixed(2);
                const transaction_id = transaction.id;
            ...
            $("#transaction_subtotal_"+index).val((parseFloat($("#transaction_subtotal_"+index).val().replace(/\,/g,'')) - remaining_balance).toFixed(2));
            $("#spent_receive_money_"+index).val((parseFloat($("#spent_receive_money_"+index).val()) - remaining_balance).toFixed(2));
            
            let total_matched = (parseFloat($("#bank_transaction_amount_"+index).val().replace(/\,/g,'')) - parseFloat($("#spent_receive_money_"+index).val().replace(/\,/g,''))).toFixed(2);
            $("#total_matched_"+index).val(total_matched);
    ...
    function minorAdjustment(rowIndex)
    {
        ...
        if(transaction_minor_adjustment <= 0 || transaction_minor_adjustment >= 0)
        {
            ...
            let total_matched = (parseFloat($("#bank_transaction_amount_"+rowIndex).val().replace(/\,/g,'')) - transaction_subtotal).toFixed(2);
