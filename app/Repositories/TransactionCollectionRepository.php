...
class TransactionCollectionRepository implements TransactionCollectionRepositoryInterface
{
    ...
    public function getTransaction($filters, $userId)
    {
        try{
            ...
            {
                return $this->transactionCollectionModel::with(['transactions', 'transactions.chartAccountsParticulars', 'transactions.invoiceTaxRates', 'payment','payment.reconcileTransaction' => function($q){
                                $q->where('is_active', 1);
                        }])
            ...
            if($type == 'edit')
            {
                ...
                if($transactionCollections['transaction_type'] != 'invoice' && $transactionCollections['transaction_type'] != 'expense' )
                {
                    $payment = Payment::create([
                        'account_id' => $transactionCollections['bank_account_id'],
                        'payment_date' => $transactionCollections['issue_date'],
                        'total_amount' => $transactionCollections['total_amount'],
                        'reference_id' => $invoice->id,
                    ]);
                    if($payment)
                    {
                        TransactionCollectionPayment::create([
                            'payment_id' => $payment->id,
                            'transaction_collection_id' => $invoice->id,
                            'amount_due' => $transactionCollections['total_amount'],
                            'payment' => $transactionCollections['total_amount']
                        ]);
    ...
    public function getTransactionsByStatus($userId, $status, $bankAccountId)
    {
        ...
        $response = $this->transactionCollectionModel::with(['payment','payment.reconcileTransaction'])->where('user_id', $userId)->where('is_active', 1);
                $response->where(function($q){
                    ...
                    $q->whereHas('payment.reconcileTransaction', function($query){
    ...
    {
        return $this->transactionCollectionModel::with('payment')->where('user_id', $userId)
    ...
    public function accountTransactions($userId, $filter){
        $response = Payment::with(['paymentTransaction', 'reconcileTransaction', 'transactionCollection'])
            ->where('account_id', $filter['bank_account_id'])
            ->whereHas('paymentTransaction',
                function ($query) use ($filter, $userId) {
                    $query->where(function ($q) use ($userId, $filter) {
                        $q->where('is_active', 1);
            ...
            ->whereHas('reconcileTransaction', function($query) use($filter,$userId) {
                $query->where(function($q) use ($userId) {
                    $q->where('is_active', 1);
                    $q->where('user_id', $userId);
                });
            });
            return $response->paginate($filter['ipp']);
...
//        return $response->paginate($filter['ipp']);
    ...
    {
        return Payment::with(['transactionCollection', 'reconcileTransaction',
                    'transactionCollection.transactions', 'transactionCollection.transactions.chartAccountsParticulars', 'transactionCollection.transactions.invoiceTaxRates'])
            ->where('account_id', $filter['bank_account_id'])
            ->whereHas('transactionCollection', function($query) use($userId) {
                $query->where('user_id', $userId)->where('is_active', 1);
            })
            ->whereHas('reconcileTransaction', function($query) use($filter){
                $query->where('is_active', 1);
            })
            ->where('reference_id', $filter['reconcileId'])
            ->first();

//        $response = $this->transactionCollectionModel::with(['payment'])
//            ->whereHas('payment', function($query) use($filter) {
//                $query->where('reference_id', $filter['reconcileId']);
//            })
//            ->first();
//
//
//        $response =  $this->transactionCollectionModel::with(['reconcileTransaction', 'transactions', 'transactions.chartAccountsParticulars', 'transactions.invoiceTaxRates'])
