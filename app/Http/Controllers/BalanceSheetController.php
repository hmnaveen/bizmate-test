...
class BalanceSheetController extends Controller
{
    ...
    public function index(Request $request) {
        ...
        // die();
        $pagedata['start_date'] = $request->start_date ? Carbon::createFromFormat('Y-m-d', $request->start_date)->format('d/m/yy') : '';
        $pagedata['end_date'] = $request->end_date ? Carbon::createFromFormat('Y-m-d', $request->end_date)->format('d/m/yy') : '';
    ...
    private function getTransactions($request, $userinfo, $start_date, $end_date, &$items_metadata, $display_date)
    {
        ...
        $transaction_reponse = SumbChartAccounts::selectRaw('sumb_chart_accounts.id as chart_accounts_id, sumb_chart_accounts.*, sumb_chart_accounts_type.id as chart_account_type_id, sumb_chart_accounts_type.*, sumb_chart_accounts_particulars.id as chart_account_parts_id, 
            ...
            foreach($transaction_reponse as $transaction){
                ...
                if(!isset($transactions[$transaction['chart_accounts_id']])){
                    $transactions[$transaction['chart_accounts_id']] = [
                        ...
                        'start_date' => Carbon::createFromFormat('Y-m-d', $start_date)->format('d/m/yy'),
                        'end_date' => Carbon::createFromFormat('Y-m-d', $end_date)->format('d/m/yy'),
                        'chart_accounts_types' => [], 
                    ];
                }

                //account_types
                if(!in_array($transaction['chart_account_type_id'], array_column($transactions[$transaction['chart_accounts_id']]['chart_accounts_types'], 'chart_account_type_id'))){
                    $transactions[$transaction['chart_accounts_id']]['chart_accounts_types'][$transaction['chart_account_type_id']] = [
                        'chart_account_type_id' => $transaction['chart_account_type_id'],
                ...
                if(!isset($transactions[$transaction['chart_accounts_id']]['chart_accounts_types'][$transaction['chart_account_type_id']]['chart_account_particulars'][$transaction['chart_account_parts_id']]))
                {
                    $transactions[$transaction['chart_accounts_id']]['chart_accounts_types'][$transaction['chart_account_type_id']]['chart_account_particulars'][$transaction['chart_account_parts_id']] = ['chart_account_parts_id' => $transaction['chart_account_parts_id'], 'chart_accounts_particulars_name' => $transaction['chart_accounts_particulars_name'], 'chart_accounts_particulars_code' => $transaction['chart_accounts_particulars_code'], 'parts_amount' => 0];
                }

                $transactions[$transaction['chart_accounts_id']]['chart_accounts_types'][$transaction['chart_account_type_id']]['chart_account_particulars'][$transaction['chart_account_parts_id']]['parts_amount'] += $transaction['parts_amount'];
                ...
                if(!isset($items_metadata['accounts'][$transaction['chart_accounts_id']]['chart_accounts_types'][$transaction['chart_account_type_id']])){
                    $items_metadata['accounts'][$transaction['chart_accounts_id']]['chart_accounts_types'][$transaction['chart_account_type_id']] = [
                        'chart_account_type_id' => $transaction['chart_account_type_id'],
                        'chart_accounts_type' => $transaction['chart_accounts_type']
                    ];
                }

                if(!isset($items_metadata['accounts'][$transaction['chart_accounts_id']]['chart_accounts_types'][$transaction['chart_account_type_id']]['chart_account_particulars'][$transaction['chart_account_parts_id']])){
                    $items_metadata['accounts'][$transaction['chart_accounts_id']]['chart_accounts_types'][$transaction['chart_account_type_id']]['chart_account_particulars'][$transaction['chart_account_parts_id']] = ['chart_account_parts_id' => $transaction['chart_account_parts_id'], 'chart_accounts_particulars_name' => $transaction['chart_accounts_particulars_name'], 'chart_accounts_particulars_code' => $transaction['chart_accounts_particulars_code']];
                }
            }
        }
