...
class ExpenseController extends Controller {
                    ...
                    $expense_number = $request->search_number_name_amount;
                        ...
                        $expensedata = TransactionCollections::where('user_id', $userinfo[0])
                                ...
                                ->orWhere(function($query) {
                                    return $query->where('transaction_type', 'spend_money')
    ...
    public function saveExpense(Request $request) {
        ...
        return redirect()->route('expense', ['err'=>1]);
    }
    ...
    public function editExpense(Request $request)
    {
        ...
        if (empty($transaction_exists)) {
            return redirect()->route('expense')->with('error', 'Transaction does not exists');
        }
    ...
    public function updateExpense(Request $request)
    {
        ...
        // $get_exp_settings = SumbExpenseSettings::where('user_id', $userinfo[0])->first()->toArray();
        $transaction = TransactionCollections::where('user_id', $userinfo[0])->where('id', $id)->first();
    ...
    public function statusChange(Request $request) {
        ...
        if($request->expense_id && $request->status){
            ...
            if($status_updated){
                return redirect()->route('expense', ['err'=>3]);
            }
            return redirect()->route('expense', ['err'=>5]);
        }
    }
